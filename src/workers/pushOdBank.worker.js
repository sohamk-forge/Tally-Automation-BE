...
const worker = new Worker(
  ...
  async (job) => {
    ...
    const result = await pool.query(
      `SELECT * FROM ${DB_SCHEMA}.bank_od_accounts WHERE id = $1`,
      [odBankId]
    );
    ...
    await pool.query(
      `UPDATE ${DB_SCHEMA}.bank_od_accounts SET sync_status = 'processing', updated_at = NOW() WHERE id = $1`,
      [odBankId]
    );
    ...
    try {
      ...
      const pairingResult = await pool.query(
        `
        SELECT cpt.user_id
        FROM ${DB_SCHEMA}.bank_od_accounts boa
      ...
      await pool.query(
        `UPDATE ${DB_SCHEMA}.bank_od_accounts SET sync_status = 'pending', updated_at = NOW() WHERE id = $1`,
        [odBankId]
      );
      ...
      if (isTemporaryOdBankError(error)) {
        await pool.query(
          `UPDATE ${DB_SCHEMA}.bank_od_accounts SET sync_status = 'pending', error_message = $1, updated_at = NOW() WHERE id = $2`,
      ...
      await pool.query(
        `UPDATE ${DB_SCHEMA}.bank_od_accounts SET sync_status = 'failed', error_message = $1, updated_at = NOW() WHERE id = $2`,
        [error.message, odBankId]
      );
