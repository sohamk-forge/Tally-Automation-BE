...
const worker = new Worker(
  ...
  async (job) => {
    ...
    try {
      ...
      // Mark as processing
      await pool.query(
        `UPDATE app_test.sales_invoice_extractions SET sync_status = 'processing' WHERE id = $1`,
        [invoiceId]
      );
      ...
      const pairingResult = await pool.query(
        ...
        JOIN app_test.companies c ON sie.company_id = c.id
      ...
      await pool.query(
        `UPDATE app_test.sales_invoice_extractions SET sync_status = 'pending' WHERE id = $1`,
        [invoiceId]
      );
      ...
      await pool.query(
        `UPDATE app_test.sales_invoice_extractions SET sync_status = 'failed', error_message = $1 WHERE id = $2`,
